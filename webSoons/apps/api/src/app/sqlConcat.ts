...
import { SqlWhere, SqlFrom, SqlInsert, SqlInsertSelect, SqlUpdate, SqlDelete, SqlProcedure, ChatObtencion } from '@Soons/interfaces-sql';
...
export class ConcatSqlService {
    ...
    hacerChat(chat: ChatObtencion) {
        ...
        return "SELECT * FROM (SELECT * from mensajes WHERE chat = " + idChat + (timestamp !== null && timestamp !== undefined ? " and orden < " + new Date(timestamp).getTime() : "") + " order by id desc limit 30) as tabla order by id asc"
    }
